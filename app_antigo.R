...
dados <- read_csv("bancos/covid/dados_covid_poa_01_05.csv") 

leitos <- read_csv("bancos/leitos/leitos_poa_01_05.csv") 
  ...
  dashboardBody(
    tabItems(
      tabItem("casos_poa",
              fluidPage(
                fluidRow(
                  column(
                    width = 6,
                    h1("Casos em Porto Alegre por fonte notificadora"),
                    dateRangeInput(
                      "datas",
      ...
      tabItem("leitos_adulto",
              fluidPage(
                h2("Escolha se os gráficos devem conter leitos de UTI e/ou emergência"),
                br(),
                ...
                fluidRow(
                  ...
                  column(
                    width = 6,
                    box(
                      title = "Número de leitos com pacientes COVID-19 por hospital",
                      background = "green",
...
server <- function(input, output) {
  ...
  output$box_ativ <- renderValueBox({
    ...
    ativos <- ifelse(total+sem_fonte-15-288 < 0,0,total+sem_fonte-15-288)
  ...
  output$barras_hosp_covid <- renderPlotly({
    ...
    p <- ggplot(aux, aes(x = local, y = leitos_covid)) +
      ...
      labs(x = "Local", y = "Número de leitos com pacientes COVID-19") +
      scale_x_discrete(limits = ordem) +
      coord_flip()
